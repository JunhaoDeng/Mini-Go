...
class MinMaxPlayer:
    ...
    def evaluate_shape(self, board, piece_type):
        ...
        patterns = {
            ...
            # "opponent_four": [[self.opponent(piece_type), self.opponent(piece_type), self.opponent(piece_type), self.opponent(piece_type), 0]],
            # "opponent_three": [[self.opponent(piece_type), self.opponent(piece_type), self.opponent(piece_type), 0, 0]],
    ...
    def min_max_ab_pruning(self, go, cur_player, piece_type, alpha, beta, depth):
        ...
        for move in self.heuristic_move_order(go, piece_type):
            if go.valid_place_check(*move, piece_type):
                ...
                if self.can_merge_chains(new_go.board, *move, self.opponent(piece_type)):
                    block_merge_bonus = 5
                else:
                    block_merge_bonus = 0
                ...
                if cur_player == 0:
                    final_score = score + factor * captured_pieces_count - threatened_pieces_score + chain_liberties_diff * 2 + new_go.score(
                        piece_type) - new_go.score(self.opponent(piece_type)) + block_merge_bonus + merge_bonus
                    if final_score > max_eval:
                        max_eval, best_move = final_score, move
                        alpha = max(alpha, final_score)
                else:
                    final_score = score - factor * captured_pieces_count + threatened_pieces_score - chain_liberties_diff * 2 + new_go.score(
                        self.opponent(piece_type)) - new_go.score(piece_type) - block_merge_bonus - merge_bonus
                    if final_score < max_eval:
                        max_eval, best_move = final_score, move

...
class MinMaxPlayer:
    ...
    def calculate_heuristic(self, go, move, piece_type):
        ...
        heuristic_val += (opp_liberties_before - opp_liberties_after) * 1  # Here, the factor 2 is arbitrary, you can adjust it
        ...
        heuristic_val += (my_liberties_after - my_liberties_before) * 1
    ...
    def chains_count(self, board, piece_type):
        visited = set()
        chains = 0
    ...
    def count_chains(self, board, i, j, direction=None, visited=None):
        if visited is None:
            visited = set()
        ...
        # 如果没有指定方向（即这是递归的首次调用），则考虑两个主要方向：垂直和水平
        if direction is None:
            directions = [(1, 0), (0, 1)]  # Down (vertical) and right (horizontal)
        else:
        ...
        for x, y in directions:
            ni, nj = i + x, j + y
            if 0 <= ni < 5 and 0 <= nj < 5 and board[ni][nj] == piece_type:
                chain_count += self.count_chains(board, ni, nj, (x, y), visited)
    ...
    def min_max_ab_pruning(self, go, cur_player, piece_type, alpha, beta, depth):
        ...
        for move in self.heuristic_move_order(go, piece_type):
            if go.valid_place_check(*move, piece_type):
                ...
                # 检查该步棋如果是对方的，是否会形成3个或以上的连锁
                penalty = 0
                new_go.board[move[0]][move[1]] = self.opponent(piece_type)
                chain_count_if_opponent_plays = self.count_chains(new_go.board, move[0], move[1])
                if piece_type == 1:
                    if chain_count_if_opponent_plays == 3:
                        penalty = 5
                    elif chain_count_if_opponent_plays >= 4:
                        penalty = 50
                else:
                    penalty = 0
                # elif chain_count_if_opponent_plays >= 5:
                #     penalty = 30
                ...
                if cur_player == 0:
                    final_score = score + factor * captured_pieces_count - threatened_pieces_score + chain_liberties_diff * 2 + new_go.score(
                        piece_type) - new_go.score(self.opponent(piece_type)) + penalty
                    if final_score > max_eval:
                        max_eval, best_move = final_score, move
                        alpha = max(alpha, final_score)
                else:
                    final_score = score - factor * captured_pieces_count + threatened_pieces_score - chain_liberties_diff * 2 + new_go.score(
                        self.opponent(piece_type)) - new_go.score(piece_type) - penalty
                    if final_score < max_eval:
                        max_eval, best_move = final_score, move

...
class MinMaxPlayer:
    ...
    def min_max_ab_pruning(self, go, cur_player, piece_type, alpha, beta, depth):
        ...
        for move in self.heuristic_move_order(go, piece_type):
            if go.valid_place_check(*move, piece_type):
                ...
                enemy_best_move_value = self.evaluate_enemy_moves(new_go,
                                                                  piece_type)  # Get the best move value for the enemy.
                if cur_player == 0:
                    final_score = (score + factor * captured_pieces_count
                           - threatened_pieces_score
                           + opponent_threatened_pieces_score
                           + chain_liberties_diff * 3
                           + new_go.score(piece_type)
                           - new_go.score(self.opponent(piece_type))
                           - enemy_best_move_value * 3)  # subtracting the enemy's best move value
                    ...
                    final_score = (score - factor * captured_pieces_count
                           ...
                           + new_go.score(self.opponent(piece_type))
                           - new_go.score(piece_type)
                           + enemy_best_move_value)  # adding the enemy's best move value
                    if final_score < max_eval:
                        max_eval, best_move = final_score, move
    ...
    def evaluate_enemy_moves(self, go, piece_type):
        """Evaluate the best move value for the enemy."""
        best_score = -math.inf
        for move in self.move_order:
            if go.valid_place_check(*move, self.opponent(piece_type)):
                temp_go = deepcopy(go)
                temp_go.place_chess(*move, self.opponent(piece_type))
                score = self.calculate_heuristic(temp_go, move, self.opponent(piece_type))

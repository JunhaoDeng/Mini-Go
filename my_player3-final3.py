...
class MinMaxPlayer:
    ...
    def evaluate_break_chain(self, go, move, piece_type):
        new_go = deepcopy(go)
        new_go.place_chess(*move, piece_type)

        before_break_value = sum(
            1 for chain_size in self.all_chain_liberties(go.board, self.opponent(piece_type)).values() if
            chain_size == 1)
        after_break_value = sum(
            1 for chain_size in self.all_chain_liberties(new_go.board, self.opponent(piece_type)).values() if
            chain_size == 1)

        return after_break_value - before_break_value
    ...
    def min_max_ab_pruning(self, go, cur_player, piece_type, alpha, beta, depth):
        ...
        for move in self.heuristic_move_order(go, piece_type):
            if go.valid_place_check(*move, piece_type):
                ...
                break_chain_score = self.evaluate_break_chain(go, move, piece_type) * 0
                if cur_player == 0:
                    final_score = score + factor * captured_pieces_count - threatened_pieces_score + opponent_threatened_pieces_score + chain_liberties_diff * 3 + new_go.score(
                        piece_type) - new_go.score(self.opponent(piece_type)) + break_chain_score * 3
                    if final_score > max_eval:
                        max_eval, best_move = final_score, move
                        alpha = max(alpha, final_score)
                else:
                    final_score = score - factor * captured_pieces_count + threatened_pieces_score - opponent_threatened_pieces_score - chain_liberties_diff * 3 + new_go.score(
                        self.opponent(piece_type)) - new_go.score(piece_type) - break_chain_score * 3
                    if final_score < max_eval:
                        max_eval, best_move = final_score, move
